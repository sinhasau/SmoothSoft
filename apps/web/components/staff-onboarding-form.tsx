...
export function StaffOnboardingForm({ pending, error, payModels = [], jobRoles = [], onCancel, onSubmit }: { pending: boolean; error?: string | null; payModels?: CustomPayModel[]; jobRoles?: CustomJobRole[]; onCancel: () => void; onSubmit: (payload: NewStaffPayload) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 px-4 py-12 backdrop-blur-[2px]" onMouseDown={onCancel}><form aria-label="New team member onboarding" className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}><div className="flex items-start justify-between border-b border-black/10 px-6 py-5"><div><h2 className="text-lg font-semibold">Add team member</h2><p className="mt-1 text-sm text-gray-500">Add the details needed for scheduling, payroll preparation, and compliance.</p></div><button type="button" aria-label="Close onboarding form" className="text-2xl leading-none text-gray-400 hover:text-black" onClick={onCancel}>×</button></div><div className="space-y-6 px-6 py-5">
  ...
  </div><div className="flex justify-end gap-2 border-t border-black/10 px-6 py-4"><Button onClick={onCancel} disabled={pending}>Cancel</Button><Button type="submit" variant="solid" disabled={pending || !identity.fullName.trim()}>{pending ? 'Creating…' : 'Create team member'}</Button></div></form></div>;
}
